...
-- Улучшенная система индикаторов
local data, i = {}, -1
...
function catch(ind)
    if string.find(ind.text, '-%d+ HP') then 
        local hp = ind.text:match('%d+') 
        ind.text = ' Safe (-' .. hp ..'HP)'
        ind.r, ind.g, ind.b = 145, 135, 255 
    end
...
local specials = {
    ['DT'] = function(v)
        ...
        if not dt.padding or (client.timestamp() - dt.padding.last > dt.padding.wait) then 
            dt.padding = {
                last = client.timestamp(), 
                value = client.random_int(3, 5), 
                wait = client.random_int(200, 400)
            } 
        end
        ...
        renderer.text(7.5 - dt.padding.value - shadow, y + dt.padding.value - shadow, 0, 0, 0, v.a, 'b+', 0, '<<')
        renderer.text(7.5 + size - 6 + dt.padding.value - shadow, y - dt.padding.value - shadow, 0, 0, 0, v.a, 'b+', 0, '>>')

        renderer.text(7.5 - dt.padding.value, y + dt.padding.value, dt.color.r, dt.color.g, dt.color.b, v.a, 'b+', 0, '<<')
        renderer.text(7.5 + size - 6 + dt.padding.value, y - dt.padding.value, dt.color.r, dt.color.g, dt.color.b, v.a, 'b+', 0, '>>')
    ...
    ['MD'] = function(v) v.text = ' MD' v.r, v.g, v.b = 145, 135, 255 end,
    ['FS'] = function(v) v.text = ' FS' v.r, v.g, v.b = 145, 135, 255 end,
    ['OSAA'] = function(v) v.text = ' OSAA' v.r, v.g, v.b = 145, 135, 255 end,
    ['DA'] = function(v) v.text = ' DA' v.r, v.g, v.b = 145, 135, 255 end,
    ['DUCK'] = function(v) v.text = ' DUCK' v.r, v.g, v.b = 145, 135, 255 end,
    ['BODY'] = function(v) v.text = ' BODY' v.r, v.g, v.b = 145, 135, 255 end,
    ['LC'] = function(v) v.text = " LC" v.r, v.g, v.b = 145, 135, 255 end,
    ['A'] = function(v) v.text = ' A Site' v.r, v.g, v.b = 145, 135, 255 end,
    ['B'] = function(v) v.text = ' B Site' v.r, v.g, v.b = 145, 135, 255 end,
    ['a_site'] = {function(v) v.text = ' A Site ' .. string.sub(v.original_text, 2) v.r, v.g, v.b = 145, 135, 255 end, '^A - '},
    ['b_site'] = {function(v) v.text = ' B Site ' .. string.sub(v.original_text, 2) v.r, v.g, v.b = 145, 135, 255 end, '^B - '},
...
function draw_indicators()
    ...
    local remove_queue = {}
    for i, v in pairs(data) do
        if not v.updated and v.a <= 5 then 
...
WIDTH = 6
HEIGHT = (screen_height / 2) + screen_height / 12
TIME_TO_PLANT_BOMB = 3
INDICATOR_TEXT_GAP = 36
OUTER_CIRCLE_RADIUS = 6
OUTER_CIRCLE_THICKNESS = OUTER_CIRCLE_RADIUS / 2
INNER_CIRCLE_RADIUS = OUTER_CIRCLE_RADIUS - 1
INNER_CIRCLE_THICKNESS = (OUTER_CIRCLE_RADIUS - 1) / 3
