    ...
    local version = "im squirting... > < 2.1 stable"
    local cfg_tbl = {
        {
            name = "Default",
    ...
    local ui_elements = {
        ...
        ragebotik = {
            ...
            infinite_exploit = ui_checkbox(group, "\aF88BFFFF:3 ~ \aFFFFFFFF\aACAC61FFInfinite Grenade Exploit[FLASHBANG]\aFFFFFFFF"),
    ...
    ui_elements.ragebotik.infinite_exploit:set_callback(function()
        ui_elements.ragebotik.infinite_exploit:set(false)
        ui_elements.ragebotik.infinite_exploit:set_visible(true)
        ui_elements.ragebotik.infinite_exploit:set_enabled(false)
    end)
    ...
    function handle_fog_dependencies()
        local fog_correction_enabled = ui_elements.settings.fog_correction:get()
        local fps_optimization_enabled = ui_elements.settings.optimizatica:get()
        local remove_fog_enabled = aa_refs.remove_foged:get()
    
        -- Обеспечиваем корректное поведение Force Fog Correction
        if fog_correction_enabled then
            ui_elements.settings.optimizatica:set_visible(true)
            aa_refs.remove_foged:set_visible(true)
            ui_elements.settings.optimizatica:set_enabled(false)
            aa_refs.remove_foged:set_enabled(false)
        else
            ui_elements.settings.optimizatica:set_enabled(true)
            aa_refs.remove_foged:set_enabled(true)
        end
    
        if fps_optimization_enabled or remove_fog_enabled then
            ui_elements.settings.fog_correction:set_visible(true)
            ui_elements.settings.fog_correction:set_enabled(false)
        else
            ui_elements.settings.fog_correction:set_enabled(true)
        end
    end
...
textures.arrow_right:draw(7.5 + size - 6 + dt.padding.value, y - dt.padding.value, size, size, dt.color.r, dt.color.g, dt.color.b, v.a, 'f')
    ...
    ui_elements.settings.fog_correction:set_callback(handle_fog_dependencies)
    ui_elements.settings.optimizatica:set_callback(handle_fog_dependencies)
    aa_refs.remove_foged:set_callback(handle_fog_dependencies)
    handle_fog_dependencies()
