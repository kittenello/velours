...
local ui_elements = {
    ...
    ragebotik = {
        ...
        min_damage_override_checkbox = ui_checkbox(group, "\aF88BFFFF>.< ~ \aFFFFFFFFMinimum Damage Override", 0),
        min_damage_slider = ui_slider(group, "\aF88BFFFF>.< ~ \aFFFFFFFFMin Damage Value", 1, 126, 105, true, "hp"),
    },
    anti_aims = {
        label_space = ui_label(main_group, "\n\n\n"),
...
ui_elements.settings.indicator_redesign:set_callback(function()
    ...
    client.set_event_callback("paint_ui", function()
        renderer.indicator(164, 158, 229, 255, tostring(aa_refs.min_damage_ref:get()) .. " DMG")
    end)
...
function handle_min_damage(cmd)
    if not ui_elements.ragebotik.min_damage_override_checkbox:get() then
        return
    end
    if cache == nil then
        cache = aa_refs.min_damage_ref:get()
    end

    if ui_elements.ragebotik.min_damage_override_checkbox.hotkey:get() then
        aa_refs.min_damage_ref:set(ui_elements.ragebotik.min_damage_slider:get())
        
        --renderer.indicator(164, 158, 229, 255, tostring(aa_refs.min_damage_ref:get()) .. " DMG")
    else
        if cache ~= nil then
            aa_refs.min_damage_ref:set(cache)
            cache = nil
        end
    end
end

client.set_event_callback("setup_command", handle_min_damage)
...
client.set_event_callback("paint_ui", handle_min_damage)
